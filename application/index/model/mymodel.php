 ...
 */
 ...
 class mymodel extends Model{
    ...
    */
    ...
    /**根据测试id得到所有的测试分数
     * Id,studentId,score,sno,name
     */
    public static function get_answer($tid)
    {
      $answer_res=answerModel::get_answer_by_testid($tid);
      $arr=array();
      $temp=array();
      foreach($answer_res as $row)
      {
        $student_res=studentModel::get_student_byId($row["studentId"]);
        $temp["Id"]=$row["Id"];
        $temp["studentId"]=$row["studentId"];
        $temp["score"]=$row["score"];
        $temp["sno"]=$student_res["sno"];
        $temp["name"]=$student_res["name"];
        $arr[]=$temp;
      }
      return $arr;
    }
